...
class TensorDataset(Dataset):
    ...
    def __getitem__(self, index):
        if len(self.tensors) == 1:
            ...
            # (self.tensors[0][index], self.tensors[1][index], ...)
                ...
                if type(items[i]) is list:
                    ...
                    for item in items[i]:
                        if fdicts is None:
                            ...
                            X_temp, fd_temp, rd_temp = __to_numerical__(
                                df[item].to_list(), one_hot=False, fdict=fdicts[i][j]
                            )
                        X.append(X_temp)
                        fd.append(fd_temp)
                        rd.append(rd_temp)
                ...
                if type(items[i]) is list:
                    ...
                    for item in items[i]:
                        if fdicts is None:
                            ...
                            X_temp, fd_temp, rd_temp = __to_numerical__(
                                df[item].to_list(), one_hot=True, fdict=fdicts[i][j]
                            )
                        X.append(X_temp)
                        fd.append(fd_temp)
                        rd.append(rd_temp)
...
def show_single_image(data, title="no_title", sec=0, save_fig=False, save_only=False, save_dir="./"):
    ...
    img = (255 * np.minimum(np.ones(img.shape), np.maximum(np.zeros(img.shape), img))).astype(
        np.uint8
    )  # 画素値を 0～1 に丸め込んだ後で 255 倍する
...
class ImagesetWriter:
    ...
    def __call__(self, data, mode="sigmoid"):
        ...
        for i in range(len(data)):
            cv2.imwrite(
                os.path.join(self.root, self.dirname, "{0:07}.{1}".format(self.curr_index, self.ext)),
                cv2.cvtColor(data[i], code=cv2.COLOR_RGB2BGR),
            )
            print(
                os.path.join(self.dirname, "{0:07}.{1}".format(self.curr_index, self.ext)).replace("\\", "/"),
                file=self.f,
            )
            self.curr_index += 1
