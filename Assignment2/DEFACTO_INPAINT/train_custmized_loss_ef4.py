...
MODEL_FILE = os.path.join(MODEL_DIR, 'UnetPlusPlus_DiceBCE_ef4_model.pth')
METRICS_DIR = './metrics/'
...
ENCODER_WEIGHTS = "imagenet"  # スクラッチから学習するため、事前学習済みの重みを使用しない
ACTIVATION = "sigmoid"
...
plt.tight_layout()
plt.savefig(os.path.join(METRICS_DIR, 'training_DiceBCE_ef4_metrics.png'))
plt.close()

print(f"メトリクスのグラフを {os.path.join(METRICS_DIR, 'training_DiceBCE_ef4_metrics.png')} に保存しました。")
